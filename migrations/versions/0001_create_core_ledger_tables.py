...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text(),
            sa.Computed(
                "CASE WHEN account_type IN ('asset', 'expense') THEN 'debit'::text ELSE 'credit'::text END",
                persisted=True,
            ),
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            ...
            name=op.f("ck_accounts_account_type_known"),
        ),
        sa.CheckConstraint(
            "currency = 'USD'", name=op.f("ck_accounts_currency_supported")
        ),
        sa.CheckConstraint(
            "length(trim(name)) > 0", name=op.f("ck_accounts_name_not_blank")
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "posted_debits", sa.BigInteger(), server_default=sa.text("0"), nullable=False
        ),
        sa.Column(
            "posted_credits", sa.BigInteger(), server_default=sa.text("0"), nullable=False
        ),
        sa.Column(
            "entry_count", sa.BigInteger(), server_default=sa.text("0"), nullable=False
        ),
        ...
        sa.CheckConstraint(
            "entry_count >= 0", name=op.f("ck_account_balances_entry_count_non_negative")
        ),
        sa.CheckConstraint(
            "posted_credits >= 0",
            name=op.f("ck_account_balances_posted_credits_non_negative"),
        ),
        sa.CheckConstraint(
            "posted_debits >= 0",
            name=op.f("ck_account_balances_posted_debits_non_negative"),
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "currency = 'USD'", name=op.f("ck_transfers_currency_supported")
        ),
        sa.CheckConstraint("amount > 0", name=op.f("ck_transfers_amount_positive")),
        sa.CheckConstraint(
            "source_account_id <> destination_account_id",
            name=op.f("ck_transfers_distinct_accounts"),
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "currency = 'USD'", name=op.f("ck_ledger_entries_currency_supported")
        ),
        sa.CheckConstraint(
            "direction IN ('debit', 'credit')",
            name=op.f("ck_ledger_entries_direction_known"),
        ),
    ...
    op.create_index(
        "ix_ledger_entries_account_id_id",
        "ledger_entries",
        ["account_id", "id"],
        unique=False,
    )
    op.create_index(
        "ix_ledger_entries_transfer_id", "ledger_entries", ["transfer_id"], unique=False
    )
