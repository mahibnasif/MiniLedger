...
def upgrade() -> None:
    ...
    sa.CheckConstraint('length(idempotency_key) BETWEEN 1 AND 255', name=op.f('ck_idempotency_keys_key_length_sane')),
    sa.CheckConstraint('length(request_hash) = 64', name=op.f('ck_idempotency_keys_request_hash_is_sha256')),
    sa.CheckConstraint('num_nonnulls(transfer_id, response_status, response_body, completed_at) IN (0, 4)', name=op.f('ck_idempotency_keys_completion_is_all_or_nothing')),
    sa.ForeignKeyConstraint(['transfer_id'], ['transfers.id'], name='fk_idempotency_keys_transfer_id', ondelete='RESTRICT'),
    sa.PrimaryKeyConstraint('endpoint', 'idempotency_key', name=op.f('pk_idempotency_keys')),
