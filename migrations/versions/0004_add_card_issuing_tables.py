...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "length(stripe_card_id) > 0", name=op.f("ck_cards_stripe_card_id_not_blank")
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "(decision = 'approved' AND transfer_id IS NOT NULL                       AND decline_reason IS NULL) OR (decision = 'declined' AND transfer_id IS NULL                       AND decline_reason IS NOT NULL)",
            name=op.f("ck_card_authorizations_decision_matches_outcome"),
        ),
        sa.CheckConstraint(
            "decision IN ('approved', 'declined')",
            name=op.f("ck_card_authorizations_decision_known"),
        ),
        sa.CheckConstraint(
            "amount > 0", name=op.f("ck_card_authorizations_amount_positive")
        ),
        sa.ForeignKeyConstraint(
            ["card_id"],
            ["cards.id"],
            name="fk_card_authorizations_card_id",
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["transfer_id"],
        ...
        sa.PrimaryKeyConstraint(
            "stripe_authorization_id", name=op.f("pk_card_authorizations")
        ),
        sa.UniqueConstraint(
            "transfer_id", name=op.f("uq_card_authorizations_transfer_id")
        ),
    )
    op.create_index(
        "ix_card_authorizations_card_id", "card_authorizations", ["card_id"], unique=False
    )
    op.create_index(
        "ix_card_authorizations_created_at",
        "card_authorizations",
        ["created_at"],
        unique=False,
    )
